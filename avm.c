...
void push(memcell val) {
    if(stack_top >= AVM_STACKSIZE) {
        printf("Fatal Error. Stack overflow. Exiting...\n");
        exit(-1);
    }
    stack_top++;
    stack[stack_top] = val;
...
memcell pop(void) {
    if(stack_top < 0) {
        printf("Fatal Error. Stack underflow. Exiting...\n");
        exit(-1);
    }
...
memcell* avm_translate_operand(vmarg* arg, memcell* reg) {
	switch(arg->type) {
		case ARG_LOCAL: return &stack[0];
		case ARG_GLOBAL: return &stack[0];
		case ARG_FORMAL: return &stack[0];
        case ARG_TEMPORARY: return &stack[0];
		case ARG_NUMBER:
            reg->type = MEM_NUMBER;
